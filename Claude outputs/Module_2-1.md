...
## Task 2.5 — Learn: Prototype — choosing a fidelity

### Task 2.5 — Step 1
**Question:** Read IxDF *Prototyping* and NN/g *Low-Fidelity vs High-Fidelity Prototyping*.

**Solution:** Completed.

### Task 2.5 — Step 2
**Question:** Make a table — paper sketch, wireframe, clickable wireframe, hi-fi interactive — and for each, note what question it answers, what it costs, and what feedback it attracts.

**Solution:**

| Fidelity | What question it answers | What it costs | What feedback it attracts |
|---|---|---|---|
| Paper sketch | Does the underlying concept make sense at all, before investing any time in making it look good? | Pen, paper, manual effort — cheap and fast to throw away and redo | Big-picture reactions to whether the idea works, not comments on visual details — "this makes sense" or "this doesn't solve the problem," not "the spacing is off" |
| Wireframe | Is the layout — where elements sit relative to each other — sensible, independent of color or style? | Static and deliberately grayscale/uncolored, so viewers aren't distracted by visual polish; more design time than a sketch but still low | Suggestions on positioning of icons, spacing, and layout — not on color or interaction |
| Clickable wireframe | Does the flow between screens/states feel right — does navigating through the product make sense? | Time and a prototyping-tool skill set to wire up interactivity; still cheap enough to redo easily if the flow is wrong | Comments on navigation and interaction — whether buttons and transitions behave the way users expect |
| Hi-fi interactive | Does the actual visual design — how it looks and feels — match what the final product should be? | Full visual-design and development time/skill; the most expensive to redo since real assets and styling decisions are baked in | Spacing, contrast, hierarchy |

*Revision note:* an earlier draft described each fidelity ("sketching the skeleton," "positioning of elements") rather than the actual question it resolves, and had the wireframe's cost column inverted (listed as interactive/colored, when a wireframe is deliberately static and grayscale). Both were corrected in the version above.

### Task 2.5 — Step 3
**Question:** For each of your three PawPal directions, decide which fidelity you'd build first and write one line of justification.

**Solution:**

| Direction | Fidelity you'd build first | Justification |
|---|---|---|
| Recreate the Owner's Presence | Paper sketch | This direction rests on an untested assumption — that sensory substitutes (scent, voice, video) actually calm the dog at all — so the first thing to resolve is concept validity, not layout or interaction, before spending time on anything more polished. |
| Gradual Trust-Building Protocols | Paper sketch | The uncertainty here is about the dog's real-world behavior over a sequence of visits — does the escalation logic (short visit → longer visit → overnight) actually make sense as a trust-building protocol — which is a claim you can sanity-check by jotting down the sequence, not something app screen navigation can validate at all. |
| Caregiver Vetting & Matching for Calming Ability | Wireframe | This direction is mostly about information architecture — what data shows on a caregiver's profile, how a trust score or certification is positioned relative to other info — so the open question is layout and hierarchy, not flow or visual polish. |

*Revision note:* earlier drafts picked fidelity by what was easiest or most familiar to build (equipment layout, "it's a multi-step flow," a UI to store data) rather than by the riskiest unproven assumption in each direction. The table above reflects the corrected reasoning.

### Task 2.5 — Step 4
**Question:** Ask Claude to challenge you: *"For each of these three, argue that I've chosen the wrong fidelity."* Note where the challenge lands.

**Solution:**

- **Recreate the Owner's Presence (Paper sketch):** challenged on the grounds that no sketch can tell you whether a dog actually calms down from a voice, scent, or video — that's a live, real-world question no app-fidelity ladder answers; arguably needs a real experiment with an actual dog, not a prototype at all.
- **Gradual Trust-Building Protocols (Paper sketch):** challenged on the grounds that if this ships as a product, there's still a real interactive surface (a booking/scheduling flow) that a pure behavioral-theory framing ignores.
- **Caregiver Vetting & Matching (Wireframe):** challenged on the grounds that trust signals (a verified badge, a calming certification) are inherently visual and emotional, not just structural — so this might be the one direction where jumping straight to something closer to hi-fi is actually defensible.

**Where the challenge landed:** agreed with the reasoning behind all three arguments, but chose to keep the original table's fidelity choices rather than revise them.

### Task 2.5 — Step 5
**Question:** Journal the rule you'll apply going forward.

**Solution:** *Rule: Before picking a fidelity, name the single specific uncertainty you're actually trying to resolve — then ask "would this artifact even let me test that, or would it only test something that looks related?" — and pick the lowest fidelity that genuinely resolves it.* (Pinned at the top of the Journal sheet alongside the 30-minute rule and the verification rule.)

---

## Task 2.6 — Learn: Test — usability testing without a lab

### Task 2.6 — Step 1
**Question:** Read both NN/g articles and watch the demo session.

**Solution:** Completed.

### Task 2.6 — Step 2
**Question:** Write a five-task usability test script for a PawPal paper prototype — tasks phrased as goals, never as instructions.

**Solution (final):**

1. You're heading out of town this weekend and need someone to look after your dog.
2. Before booking, message a sitter to ask a question you'd want answered about how they'd care for your pet.
3. Find a sitter whose rate fits within a budget of ₹500/day.
4. Switch to the sitter side: mark yourself available this Saturday and Sunday from 9 AM to 6 PM.
5. Book a sitter and complete the payment for the service.

*Revision history:* earlier drafts used imperative "Check X" / "Perform X" phrasing instead of real scenarios; referenced a "package" feature and a "user satisfaction" reviews feature that Brief A never confirms exist; and specified credit-card payment where the brief explicitly says UPI. Task 2 and Task 4 were rewritten around confirmed brief features (messaging; sitter availability/rates) instead, and Task 5 was made payment-method-agnostic. Task 4 also went through a vagueness fix, adding the concrete "Saturday and Sunday, 9 AM–6 PM" constraint so there's a checkable completion state, the same way Task 3's ₹500/day constraint works.

### Task 2.6 — Step 3
**Question:** Ask Claude to review it: *"Here's a usability test script. Which of these tasks leak the answer, and which are too vague to complete?"*

**Solution:** 4 of 5 passed cleanly (no leaked interface vocabulary). Task 4 was flagged as too vague on completion criteria in an earlier draft ("set your availability" didn't specify what "done" looked like) — fixed by adding the specific day/time window shown above.

### Task 2.6 — Step 4
**Question:** Sketch a rough paper prototype and run the test on one colleague.

**Solution:** Built a five-screen digital wireframe (Browse/Search Sitters, Sitter's Profile, Messaging, Booking/Confirmation + Payment, Sitter-Side Availability Setup) using a literal, no-additions build constraint, then ran the actual test on a real colleague against the five tasks. The critique that came back — a header/navigation bar that didn't reflect the actual screen flow; no ratings/reviews element despite trust being the brief's central concern; and an over-specified "Become a Paid Sitter" screen treated with the same visual density as the core booking flow — came from that colleague's session.

**Concrete usability finding:** the colleague got genuinely stuck on Task 4 ("mark yourself available this Saturday and Sunday from 9 AM to 6 PM"), because the Sitter-Side Availability Setup screen has no actual day/time-setting element at all — only name, pet-ownership questions, and an animal-type checkbox list. This is exactly the kind of real breakdown a usability test is meant to surface: a genuine gap between what a task requires and what the interface actually offers.

### Task 2.6 — Step 5
**Question:** Write up: three things that worked, three things that broke, one thing you'd never have predicted.

**Solution:**

**Three things that worked:**

1. The literal, no-additions constraint kept the wireframes honest. Because the build was told exactly which elements to add and not to correct anything, the resulting screens were a faithful record of what was actually specified — which is exactly why the critique could surface real gaps (missing ratings/reviews, missing pet-type selector) instead of gaps hidden by quietly "fixing" things that weren't asked for.
2. Keeping the messaging screen minimal exposed a real product decision, not just a UI oversight. The "Click to book a call" being an optional bubble wasn't a bug — it was a faithful rendering of an ambiguous requirement, which only became visible because the wireframe didn't paper over that ambiguity with an assumption.
3. Structuring each screen as a flat list of boxes made it easy to point at exactly what's missing or extra, since there was no visual hierarchy or grouping baked in to debate.

**Three things that broke:**

1. Optimized for "match the elements given" over "match the actual user need" — ratings/reviews never made it in because they weren't in the original element list, even though trust was the #1 concern from the very first PawPal brief. Literal compliance and correctness aren't the same thing.
2. The menu bar on Screen 1 was built as a static, disconnected list of labels rather than something reflecting the full set of screens in the flow — a structural miss, not a content one, from treating each screen as an isolated unit.
3. The sitter-application screen was built with the same visual density as the core booking flow, when it's actually a rare, secondary path — nothing signaled "edge case, not core case," which invited over-specification.

**One thing that wasn't predicted:** the fix for "too much noise" and the fix for "missing ratings/reviews" are in tension on the same screens — adding trust signals is exactly the kind of addition that risks tipping a screen into clutter. It's a real trade-off, not two independent notes: the redesign has to decide where trust signals earn their space, not assume more trust info is automatically better.

*Note: alongside these three, the colleague's session also produced the concrete finding recorded under Step 4 — getting stuck on Task 4 due to the missing availability element on the Sitter-Side screen. Kept as a separate note rather than folded into the three above, at the user's choice.*
