...
*Use this when: you have local changes to a project that's already on GitHub and need the exact steps to get them pushed up safely.*

```
Role: A senior developer walking a junior teammate through updating an existing GitHub repository. Context: I have a project already cloned locally at [LOCAL PATH/REPO NAME], connected to the GitHub repo [REPO URL]. I've made changes ([BRIEF DESCRIPTION OF CHANGES]) and need to get them onto GitHub. My current setup: [BRANCH NAME, e.g. main / a feature branch], and I [HAVE / HAVEN'T] pulled the latest changes from the remote first. Task: Walk me through updating the repository step by step — checking status, staging the right files, committing with a clear message, and pushing. Before any step that could overwrite my own or someone else's work (force-push, hard reset, pulling over uncommitted changes), stop and flag it explicitly rather than just running it. Format: Numbered steps, each with the exact command and a one-line explanation of what it does. Constraints: Assume I know basic git vocabulary (commit, branch, remote) but not much beyond that. If anything about my setup above is ambiguous or missing (e.g. whether I'm behind the remote), ask before giving commands rather than guessing.
```
